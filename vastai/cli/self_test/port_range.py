...
The host installer persists its configured range locally, while the instance
API returns the allocated range once a test instance is running.  This module
keeps parsing and probing independent from the large self-test orchestration
function so both paths can be tested without renting an instance.
"""
...
def resolve_port_range(
    ...
    The local installer file is authoritative before launch because it can be
    handed to the container.  The API range is a useful fallback for
    diagnostics after launch and also lets the CLI report what the backend
    allocated when the command was run away from the host.
    """
